...
public:
    ...
    void Info(const std::string& message) { AddLog("[INFO] " + message); }
    void Warning(const std::string& message) { AddLog("[WARN] " + message); }
    void Error(const std::string& message) { AddLog("[ERR] " + message); }
    void Critical(const std::string& message) { AddLog("[CRIT] " + message); }
    
    std::vector<std::string> GetRecentLogs(size_t count) const {
        std::vector<std::string> recent;
        size_t start = logs_memory.size() > count ? logs_memory.size() - count : 0;
        recent.insert(recent.begin(), logs_memory.begin() + start, logs_memory.end());
        return recent;
    }
    
    void SaveToFile(const std::string& filename) {
        std::ofstream file(filename);
        for (const auto& log : logs_memory) {
            file << log << "\n";
        }
        file.close();
    }
